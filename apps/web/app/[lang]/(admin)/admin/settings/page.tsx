...
export default function AdminSettingsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const watchedPublicUrl = form.watch('sslczPublicUrl') || 'https://undone-unsure-twisting.ngrok-free.dev';
  ...
  const onSubmit = async (values: SettingsFormValues) => {
    try {
      ...
      if (values.sslczStorePassword && values.sslczStorePassword.trim() !== '') {
        payload.sslczStorePassword = values.sslczStorePassword.trim();
      }

      await updateSettings(payload).unwrap();
      form.setValue('sslczStorePassword', '');
      toast.success(isBn ? 'সকল সেটিংস সফলভাবে সংরক্ষিত হয়েছে' : 'Settings saved successfully');
    } catch (error) {
      toast.error(
    ...
    <div className="space-y-8 w-full pb-16">
      ...
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight">
          {isBn ? 'সিস্টেম ও গেটওয়ে সেটিংস' : 'System & Gateway Settings'}
        </h1>
        <p className="text-muted-foreground mt-1.5 text-sm sm:text-base">
          {isBn
            ? 'মার্কেটপ্লেস কনফিগারেশন, SSLCOMMERZ পেমেন্ট গেটওয়ে ও লোকাল ngrok টানেল পরিচালনা করুন।'
            : 'Configure marketplace details, SSLCOMMERZ payment gateway, and active ngrok tunnel.'}
        </p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Card className="border-0 shadow-sm ring-1 ring-primary/20 rounded-2xl overflow-hidden">
            <CardHeader className="bg-primary/5 border-b border-primary/10 pb-5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl">
                      {isBn ? 'SSLCOMMERZ পেমেন্ট গেটওয়ে ও ngrok টানেল' : 'SSLCOMMERZ Gateway & ngrok Tunnel'}
                    </CardTitle>
                    <CardDescription>
                      {isBn
                ...
                >
                  <Radio className={`w-3 h-3 ${isLive ? 'text-red-500 animate-pulse' : 'text-emerald-500 animate-ping'}`} />
            ...
            <CardContent className="pt-6 space-y-6">
              ...
              <div className="p-4 rounded-xl bg-muted/50 border border-border/80 flex items-start gap-3 text-sm">
                ...
                <div className="space-y-1">
                  <p className="font-semibold text-foreground">
                    {isBn ? 'ডাইনামিক ngrok টানেল সাপোর্ট' : 'Dynamic ngrok Tunnel Support'}
                  </p>
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="font-semibold flex items-center justify-between">
                      <span>{isBn ? 'পাবলিক টানেল / কলব্যাক বেস URL' : 'Public Tunnel / Gateway Base URL'}</span>
              ...
              <div className="rounded-xl bg-slate-950 p-4 text-slate-100 font-mono text-xs space-y-2.5 border border-slate-800">
                <div className="flex justify-between items-center pb-2 border-b border-slate-800 text-slate-400">
                  <span className="flex items-center gap-1.5 font-sans font-medium text-xs">
                    ...
                    {isBn ? 'গেটওয়ে এন্ট্রিপয়েন্ট প্রিভিউ' : 'Active Gateway Endpoints Preview'}
                  </span>
                  <a
                ...
                <div className="space-y-1.5 pt-1">
                  <div className="flex items-center justify-between bg-slate-900/80 px-2.5 py-1.5 rounded border border-slate-800/80">
                    ...
                    >
                      {copiedKey === 'success' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-slate-900/80 px-2.5 py-1.5 rounded border border-slate-800/80">
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-white shrink-0"
                    >
                      {copiedKey === 'ipn' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="font-semibold">{isBn ? 'স্টোর আইডি (Store ID)' : 'Store ID'}</FormLabel>
                      <FormControl>
                        <Input {...field} placeholder="ghore6ab014aca85d1" className="font-mono text-sm" />
                      </FormControl>
                      <FormDescription className="text-xs">
                        {isBn ? 'SSLCOMMERZ মার্চেন্ট স্টোর আইডি' : 'Your SSLCOMMERZ Merchant Store ID'}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="font-semibold flex items-center justify-between">
                        <span>{isBn ? 'স্টোর পাসওয়ার্ড (Store Password)' : 'Store Password'}</span>
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start gap-3 space-y-0 p-4 rounded-xl border bg-muted/20">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel htmlFor="sslcz-is-live" className="font-semibold cursor-pointer">
                        {isBn ? 'লাইভ / প্রোডাকশন মোড সক্রিয় করুন' : 'Enable Live Production Mode'}
                      </FormLabel>
                      <FormDescription className="text-xs">
                        {isBn
          ...
          <Card className="border-0 shadow-sm ring-1 ring-black/5 rounded-2xl overflow-hidden">
            <CardHeader className="bg-muted/30 border-b pb-4">
              <CardTitle className="text-xl">{isBn ? 'জেনারেল সেটিংস' : 'General Settings'}</CardTitle>
              <CardDescription>
                {isBn ? 'প্ল্যাটফর্মের সাধারণ তথ্য ও যোগাযোগের বিবরণ' : 'Update general platform information & contact channels'}
              </CardDescription>
            </CardHeader>
          ...
          <Card className="border-0 shadow-sm ring-1 ring-black/5 rounded-2xl overflow-hidden">
            <CardHeader className="bg-muted/30 border-b pb-4">
              <CardTitle className="text-xl">{isBn ? 'মার্কেটপ্লেস কন্ট্রোল' : 'Marketplace Controls'}</CardTitle>
              <CardDescription>
                {isBn ? 'সেলার ও ব্যবহারকারী অপারেশন নিয়ন্ত্রণ করুন' : 'Control operations and registrations across the platform'}
              </CardDescription>
            </CardHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start gap-3 space-y-0">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel htmlFor="seller-reg" className="font-semibold cursor-pointer">
