...
export default function AdminRiderApplicationsPage({
  ...
  const { data, isLoading, isError, refetch } = useGetAdminRiderApplicationsQuery({
    page,
    limit,
    status: statusFilter === 'ALL' ? undefined : statusFilter,
  });

  const [approveApp, { isLoading: isApproving }] = useApproveRiderApplicationMutation();
  const [rejectApp, { isLoading: isRejecting }] = useRejectRiderApplicationMutation();
  ...
  const columns: ColumnDef<RiderApplication>[] = [
    {
      accessorKey: 'fullName',
      header: isBn ? 'নাম' : 'Full Name',
      cell: ({ row }) => (
        <div>
    ...
    },
    {
      ...
      header: isBn ? 'এনআইডি' : 'NID',
      cell: ({ row }) => <span className="font-mono text-xs">{row.original.nidNumber}</span>,
    },
    {
      accessorKey: 'preferredZone',
      header: isBn ? 'পছন্দের জোন' : 'Preferred Zone',
      cell: ({ row }) => row.original.preferredZone || '—',
    },
    {
      accessorKey: 'status',
      ...
      >
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {isBn ? 'রাইডার আবেদনের বিস্তারিত তথ্য' : 'Detailed delivery partner information'}
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedApp && (
            <div className="space-y-4 py-2 text-sm">
              <div className="grid grid-cols-2 gap-4 p-4 rounded-2xl bg-muted/40 border">
                ...
                <div>
                  <span className="text-muted-foreground text-xs block font-medium">
                    {isBn ? 'যানবাহনের ধরন' : 'Vehicle Type'}
                  </span>
                  <span className="font-semibold">{selectedApp.vehicleType}</span>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs block font-medium">
                ...
                <div>
                  <span className="text-muted-foreground text-xs block font-medium">
                    {isBn ? 'যানবাহন নম্বর' : 'Plate Number'}
                  </span>
                  <span className="font-mono">{selectedApp.vehiclePlateNumber || '—'}</span>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs block font-medium">
                    {isBn ? 'ড্রাইভিং লাইসেন্স' : 'Driving License'}
                  </span>
                  <span className="font-mono">{selectedApp.drivingLicenseNumber || '—'}</span>
                </div>
                <div>
                  <span className="text-muted-foreground text-xs block font-medium">
                    {isBn ? 'পছন্দের জোন' : 'Preferred Zone'}
                  </span>
                  <span>{selectedApp.preferredZone || '—'}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="p-3 rounded-xl bg-muted/20 border text-foreground">
                  {selectedApp.emergencyContact || (isBn ? 'দেওয়া হয়নি' : 'Not provided')}
                </p>
              </div>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder={
                  reviewAction === 'approve'
                    ? 'Documents verified. Rider active.'
                    : 'Driving license expired or illegible.'
                }
                value={adminNotes}
