  ...
  return (
    <div className="flex flex-col min-h-screen bg-muted/20">
      ...
      <nav className="fixed bottom-0 z-50 w-full border-t bg-background/80 backdrop-blur-xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)] flex items-center justify-around h-16 pb-safe">
        <NavItem href="/en/rider" icon={ClipboardList} label="Tasks" pathname={pathname} />
        <NavItem href="/en/rider/map" icon={MapIcon} label="Map" pathname={pathname} />
        <NavItem href="/en/rider/earnings" icon={Wallet} label="Earnings" pathname={pathname} />
        <NavItem href="/en/rider/profile" icon={User} label="Profile" pathname={pathname} />
      </nav>
