...
import { useGetAdminDisputesQuery, DisputeStatus } from '@/features/disputes/disputesApi';
...
export function AdminDisputesView({ lang = 'en' }: AdminDisputesViewProps) {
  ...
  const filteredDisputes = useMemo(() => {
    return disputes.filter((dispute) => {
      ...
      const customer = `${dispute.customer?.firstName || ''} ${dispute.customer?.lastName || ''}`;
      const seller = `${dispute.seller?.firstName || ''} ${dispute.seller?.lastName || ''}`;
          ...
          <div className="flex w-full flex-1 flex-col gap-4 sm:w-auto sm:flex-row sm:items-center">
            ...
            <div className="relative w-full max-w-md min-w-[200px] flex-1 sm:w-auto">
              ...
              <input
                ...
                placeholder={isBn ? 'অর্ডার, গ্রাহক, সেলার বা কারণ দিয়ে খুঁজুন...' : 'Search by order ID, customer, seller, or reason...'}
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-gray-200 bg-gray-50 uppercase text-xs font-semibold text-gray-900 tracking-wider dark:border-border dark:bg-muted/40 dark:text-foreground">
                <tr>
                  ...
                  <th className="py-3.5 px-4">{isBn ? 'সেলার' : 'Seller'}</th>
                ...
                {isLoading ? (
                  Array.from({ length: 5 }).map((_, index) => (
                    <tr key={`skeleton-${index}`} className="animate-pulse">
                      ...
                      <td className="py-4 px-4"><div className="h-4 w-24 rounded bg-muted"></div></td>
                      <td className="py-4 px-4 text-right"><div className="ml-auto h-8 w-24 rounded bg-muted"></div></td>
                    </tr>
                  ))
                ) : isError ? (
                  <tr>
                      ...
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm text-muted-foreground max-w-sm">
                          {search || statusFilter !== 'ALL'
                            ...
                            : (isBn ? 'প্ল্যাটফর্মে কোনো সক্রিয় বিরোধ নেই' : 'There are no active disputes.')}
                        </p>
                        {(search || statusFilter !== 'ALL') && (
                          <button
                            type="button"
