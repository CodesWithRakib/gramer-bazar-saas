...
| :--- | :--- | :--- | :--- | :--- |
| **Color Tokens** | Raw hex values (`#FAFAFA`, `#0A0A0A`) in `DashboardLayout.tsx` and 15+ hardcoded raw Tailwind colors (`bg-cyan-600`, `bg-red-600`, `bg-amber-600`) in `ProductCard.tsx` | High | Centralize all tints into semantic tokens (`--color-background`, `--color-muted`, `--color-accent`) defined in `globals.css` | Verified |
| **Typography** | Font stacks use Next.js fonts (`--font-inter`, `--font-noto-bengali`), but headings throughout customer and admin views use irregular text sizes (`text-2xl`, `text-3xl`, `text-xl`) without unified line-height tokens | Medium | Standardize with `<PageHeader>` supporting title, description, and breadcrumb hierarchy | Verified |
| **Spacing** | Grid gaps alternate inconsistently between `gap-3`, `gap-4`, `gap-6`, `gap-8`, and `gap-10` across dashboard pages | Medium | Standardize layout spacing on a 4-point / 8-point grid rhythm (e.g. `gap-4` for cards, `gap-6` for sections, `gap-8` for major grids) | Verified |
| **Radius** | `--radius: 0.5rem` is configured in `globals.css`, but components mix `rounded-md` (button/input), `rounded-xl` (card), and `rounded-2xl` (hero/dashboard links) | Low | Standardize corner radiuses: `rounded-md` for interactive controls (buttons, inputs), `rounded-xl` for cards, `rounded-2xl` for large banners/containers | Verified |
| **Shadows** | `shadow-xs`, `shadow-sm`, `shadow-md`, and custom drop shadows are applied haphazardly across catalog items and buttons | Medium | Standardize elevation: `shadow-xs` for subtle cards, `shadow-sm` for interactive hover cards, `shadow-lg` for modals/drawers | Verified |
| **Borders** | Some components apply both `border` and `divide-y`, while others rely on `border-border/50` or `border-border/70` opacity overrides | Low | Enforce unified `border-border` token across all card and divider containers | Verified |
| **Buttons** | `Button` component wraps with Framer Motion `motion.button` and enforces a 44px default height (`h-11`), while small variants use `h-9` | Medium | Added `loading` prop with animated spinner, `icon-sm` size, and preserved `"use client"` motion interaction | Verified |
| **Inputs** | Standard input height is 44px (`h-11`), but search inputs and filter inputs in admin tables often specify `h-9` or `h-10` with inline overrides | Medium | Added formal `inputSize` prop (`sm`, `default`, `lg`) to `Input.tsx` | Verified |
| **Badges** | `Badge` only has `default`, `secondary`, `destructive`, and `outline`, missing essential ecommerce statuses (`success`, `warning`, `info`) | High | Extended `badgeVariants` with `success` (green), `warning` (amber), and `info` (blue) variants | Verified |
| **Cards** | Clean Radix/shadcn `Card`, but custom pages wrap it in extra redundant div wrappers with conflicting background colors | Medium | Standardized card containers and dark-mode safe `bg-card` backgrounds | Verified |
| **Dialogs & Drawers** | Modals and sheets are accessible via Radix UI, but mobile dialogs lacked responsive width constraint and `window.confirm()` was used | High | Added `<ConfirmDialog>` with asynchronous states and enhanced DialogContent for mobile viewports (`w-[calc(100%-2rem)]`, `max-h-[90dvh]`) | Verified |
| **Tables** | `data-table.tsx` uses custom table layout with pagination that differs visually from `AdminPagination.tsx` | High | Standardized all admin & seller views to canonical `@/components/ui/AdminPagination` | Verified |
